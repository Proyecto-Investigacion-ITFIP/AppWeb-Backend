...
const resolversCliente= {
  ...
  Mutation: {
    ...
    editarCliente: async (parent, args) => {
      const clienteEditado = await ClienteMODEL.findByIdAndUpdate(args._id, {
        nombre: args.nombre,
        apellido: args.apellido,
        email: args.email,
        identificacion: args.identificacion,
    ...
    eliminarCliente: async (parent, args) => {
      if (Object.keys(args).includes('_id')) {
        const clienteEliminado = await ClienteMODEL.findOneAndDelete({ _id: args._id });
        return clienteEliminado;
      } else if (Object.keys(args).includes('email')) {
        const clienteEliminado = await ClienteMODEL.findOneAndDelete({ email: args.email });
        return clienteEliminado;
      }
    },
  },
